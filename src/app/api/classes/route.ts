...
export async function POST(req: NextRequest) {
  ...
  try {
    const created = await tenantTransaction(auth, async (client) => {
      const { rows: roomRows } = await client.query(
        `SELECT name, capacity FROM rooms WHERE id = $1 AND tenant_id = $2`,
        [roomId, auth.tenantId],
      );
      if (roomRows.length === 0) throw new Error("Sala inválida.");
      ...
      if (coachId) {
        ...
        if (coachRows.length === 0) throw new Error("Coach inválido.");
      ...
      return { ...rows[0], coach: coachName, booked: 0, waitlist: 0 };
    });

    return NextResponse.json(created, { status: 201 });
  } catch (err) {
    const message = err instanceof Error ? err.message : "No se pudo crear la clase.";
